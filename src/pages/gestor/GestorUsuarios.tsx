...
import { Link2, Users, Trash2, Plus } from "lucide-react";
...
export default function GestorUsuarios() {
  ...
  const handleAssign = async () => {
    if (!effectiveClientId || !assignUserId) return;
        ...
        <div className="flex flex-wrap gap-1">
          {row.roles.map((r) => (
            <Badge key={r.id} variant="outline" className="text-[10px]">
              {roleLabel(r.role)}
              {r.venue_id && ` · ${venueMap.get(r.venue_id) ?? "—"}`}
            ...
            render: (row: UserRow) => (
              <div className="flex items-center gap-1">
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    setAssignUserId(row.user_id);
                    setAssignOpen(true);
                  }}
                  title={t("gusr_add_role")}
                >
                  <Plus className="h-3.5 w-3.5" />
                </Button>
                {row.roles
                  .filter((r) => ALLOWED_ROLES.includes(r.role))
                  .map((r) => (
                    <Button
                      key={r.id}
                      size="sm"
                      ...
                      title={`${t("gusr_revoke")} ${roleLabel(r.role)}`}
                    >
      ...
      <div className="space-y-6">
        ...
        <Dialog open={assignOpen} onOpenChange={setAssignOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>{t("gusr_add_role")}</DialogTitle>
            </DialogHeader>
