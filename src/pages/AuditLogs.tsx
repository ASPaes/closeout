...
const actionColors: Record<string, string> = {
  ...
  user_created: "bg-primary/15 text-primary", user_deactivated: "bg-warning/15 text-warning",
  venue_created: "bg-primary/15 text-primary", client_created: "bg-primary/15 text-primary",
  role_assigned: "bg-primary/15 text-primary", role_revoked: "bg-destructive/15 text-destructive",
};
...
export default function AuditLogs() {
  ...
  const filtered = logs.filter((l) => (filterAction === "all" || l.action === filterAction) && (l.action.toLowerCase().includes(search.toLowerCase()) || (l.entity_type || "").toLowerCase().includes(search.toLowerCase())));
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              ...
              {filtered.length === 0 && <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-8">{t("no_logs_found")}</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
